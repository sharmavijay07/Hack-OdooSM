...
export default function AdjustmentsPage() {
    ...
    useEffect(() => {
        const fetchOperations = async () => {
            try {
                const res = await api.get('/operations?type=ADJUSTMENT');
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-card p-4 rounded-xl border border-border shadow-sm">
                <div className="flex items-center gap-4 w-full sm:w-auto">
                    <Link
                        href="/operations/new?type=ADJUSTMENT"
                            ...
                            {filteredOperations.map((op: any) => (
                                <tr key={op._id} className="hover:bg-muted/50 transition-colors">
                                    <td className="p-4 font-medium text-primary">
                                        <Link href={`/operations/${op._id}`}>{op.reference || 'WH/ADJ/XXXX'}</Link>
                                    </td>
                                    <td className="p-4 text-foreground">Main Warehouse</td>
                                    <td className="p-4 text-foreground">Stock</td>
                                    <td className="p-4 text-muted-foreground">{op.responsible || 'Admin'}</td>
                                    <td className="p-4 text-muted-foreground">{new Date(op.createdAt).toLocaleDateString()}</td>
                                    <td className="p-4">
                            ...
                            {filteredOperations.length === 0 && (
                                <tr>
                                    <td colSpan={6} className="p-8 text-center text-muted-foreground">
                                        No adjustments found. Create stock adjustments to fix inventory discrepancies.
                                    </td>
                                </tr>
                            )}
                        </tbody>
