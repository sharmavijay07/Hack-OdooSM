        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-card p-4 rounded-xl border border-border shadow-sm">
                <div className="flex items-center gap-4 w-full sm:w-auto">
                    <Link
                        href="/operations/new?type=DELIVERY"
                    ...
                    <h1 className="text-2xl font-bold text-primary">Deliveries</h1>
                </div>
                            ...
                            {filteredOperations.map((op: any) => (
                                <tr key={op._id} className="hover:bg-muted/50 transition-colors">
                                    <td className="p-4 font-medium text-primary">
                                        <Link href={`/operations/${op._id}`}>{op.reference || 'WH/OUT/XXXX'}</Link>
                                    </td>
                                    <td className="p-4">WH/Stock</td>
                                    <td className="p-4">Customer</td>
                                    <td className="p-4">{op.contact || 'Azure Interior'}</td>
                                    <td className="p-4">{new Date(op.createdAt).toLocaleDateString()}</td>
                                    <td className="p-4">
